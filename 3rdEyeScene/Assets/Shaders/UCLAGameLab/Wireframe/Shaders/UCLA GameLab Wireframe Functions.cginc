...
struct UCLAGL_g2f
{
  ...
  float4  color : COLOR;      // fragment base color
...
{			
  ...
  float4 targetColor = input.color * _Color * tex2D( _MainTex, input.uv);
  float4 transCol = input.color * _Color * tex2D( _MainTex, input.uv);
  transCol.a = 0;
